...
import { getMysqlConfig, getMysqlPool } from "./mysqlClient";
...
async function initDb(): Promise<ReturnType<typeof drizzle> | null> {
  ...
  try {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      console.log("[Database] Connected to MySQL database successfully for Drizzle ORM.");
      return drizzle(pool as any, { mode: "default" }) as any;
    }
    return null;
  } catch (error: any) {
    console.warn(
