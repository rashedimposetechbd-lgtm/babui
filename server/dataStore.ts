...
class DataStoreManager {
    ...
    try {
      ...
      if (mysqlState) {
        this.data = {
          ...this.data,
          ...mysqlState,
          settings: { ...this.data.settings, ...(mysqlState.settings || {}) },
        };
        this.persist(false); // Update local cache without re-triggering sync
      }
    } catch (e) {
      console.warn("[DataStore] Could not load state from MySQL:", e);
    }
  }

  public async refreshFromMysql(): Promise<boolean> {
    try {
      const mysqlState = await loadStateFromMysql();
      if (mysqlState) {
        this.data = {
      ...
      console.warn("[DataStore] Refresh from MySQL failed:", e);
    }
    return false;
  }
  ...
  private persist(syncMysql = true) {
    try {
    ...
    if (syncMysql) {
      try {
        triggerMysqlSync(this.data);
      } catch (e) {
        console.warn("[DataStore] Background sync trigger failed:", e);
      }
    }
  }
